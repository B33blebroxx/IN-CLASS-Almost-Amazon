    ...
    array.forEach((item) => {
      ...
      <div class="card" style="width: 18rem;">
        ...
        <p class="card-text bold">${item.favorite ? '<span class="badge rounded-pill text-bg-dark">Favorite</span>' : ''}</p>
          ...
          <hr>
          <i class="btn btn-outline-secondary" id="view-author-btn--${item.firebaseKey}">View</i>
          <i class="btn btn-outline-info" id="update-author--${item.firebaseKey}">Update</i>
          <i class="btn btn-outline-danger" id="delete-author-btn--${item.firebaseKey}">Delete</i>
        </div>
      </div>`;
    });
  }
